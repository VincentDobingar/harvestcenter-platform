...
export default function BoursePromotion({
  ...
  const resolvedTitle = title || t("scholarshipPromo.title");
  const resolvedSponsor = sponsor || t("scholarshipPromo.sponsor");
  const resolvedDescription = description || t("scholarshipPromo.description");
  const resolvedLocation = location || t("scholarshipPromo.location");
  const resolvedQuota = quota || t("scholarshipPromo.quota");
  ...
  const deadlineLabel = d.toLocaleDateString(locale, {
    day: "2-digit",
    month: "long",
    year: "numeric",
  });
  ...
  return (
    <Section
      id="promotion"
      title={t("scholarshipPromo.sectionTitle")}
      subtitle={t("scholarshipPromo.sectionSubtitle")}
      centered
    >
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand to-brand-700">
        <div
          aria-hidden
          className="absolute -top-20 -right-24 w-[28rem] h-[28rem] rounded-full bg-white/10 blur-3xl"
        />
        <div
          aria-hidden
          className="absolute -bottom-24 -left-24 w-[24rem] h-[24rem] rounded-full bg-white/10 blur-3xl"
        />
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch p-6 md:p-10">
          <div className="text-white">
            ...
            <p className="mt-3 text-white/90">{resolvedDescription}</p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              >
                {t("scholarshipPromo.applyNow")} <ExternalLink className="w-4 h-4" />
              </a>
              ...
              {daysLeft <= 10 && (
                <span className="badge-brand bg-white/15 text-white border border-white/30">
                  {daysLeft === 0
                    ? t("scholarshipPromo.lastDay")
                    : t("scholarshipPromo.daysLeft", { count: daysLeft })}
                </span>
              )}
            </div>
          </div>
        ...
        {showEmbed && embedUrl && (
          <div className="relative border-t border-white/15">
            <iframe
              title={t("scholarshipPromo.formTitle")}
              src={embedUrl}
